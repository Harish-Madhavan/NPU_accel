...
class TestAsyncAndHybrid:
    def test_asynchronous_pipeline_latency(self):
        ...
        assert isinstance(compiled_model, NPUGraphModule)
        ...
        # Check values
        assert torch.allclose(out_async, out_expected, rtol=1e-2, atol=1e-2)
    ...
    def test_asynchronous_pipeline_throughput(self):
        ...
        assert compiled_model.num_streams == 2
        assert len(compiled_model.infer_requests) == 2
        ...
        assert torch.allclose(out1, model(x1, y1), rtol=1e-2, atol=1e-2)
        assert torch.allclose(out2, model(x2, y2), rtol=1e-2, atol=1e-2)
    ...
    def test_hybrid_graph_partitioning_fallback(self):
        ...
        try:
            ...
            pytest.fail(f"Compilation failed due to NPUCompilationError: {e}")
        ...
        assert not isinstance(compiled_model, NPUGraphModule)
        assert isinstance(compiled_model, torch.fx.GraphModule)
        ...
        assert torch.allclose(out_compiled, out_expected, rtol=1e-2, atol=1e-2)
    ...
    def test_npu_async_future_submit(self):
        ...
        assert isinstance(compiled, NPUGraphModule)
        ...
        res = future.result()
        expected = model(x)
        assert torch.allclose(res.float(), expected.float(), atol=1e-2, rtol=1e-2)
    ...
    def test_batch_infer_pipeline(self):
        ...
        for i in range(6):
            ...
            assert torch.allclose(results[i].float(), expected.float(), atol=1e-2, rtol=1e-2)
