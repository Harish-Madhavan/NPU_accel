...
class TestQuantization(unittest.TestCase):
    def test_quantized_linear_eager(self):
        ...
        self.assertTrue(torch.allclose(out_eager, expected, atol=1e-2, rtol=1e-2))
        ...
        self.assertTrue(
            torch.allclose(out_eager_simple, expected_simple, atol=1e-2, rtol=1e-2)
        )
    ...
    def test_quantized_linear_compiled(self):
        ...
        try:
            ...
            self.fail(f"Quantized compilation failed: {e}")
    ...
    def test_quantized_linear_float16(self):
        ...
        # Eager execution
        out_eager = quantized_linear(input_t, weight_t, scale_t, zp_t, bias_t)
        self.assertEqual(out_eager.dtype, torch.float16)
        ...
        try:
            ...
            out_npu = npu_model(input_t, weight_t, scale_t, zp_t, bias_t)
            self.assertTrue(
                torch.allclose(out_npu.float(), out_eager.float(), atol=1e-2, rtol=1e-2)
            )
        except Exception as e:
            self.fail(f"Float16 quantized compilation failed: {e}")
    ...
    def test_quantized_linear_int4(self):
        ...
        # Expected output via reference manual unpack
        ...
        try:
            ...
            out_npu = npu_model(input_t, weight_t, scale_t, zp_t, bias_t)
            self.assertTrue(torch.allclose(out_npu, out_eager, atol=1e-2, rtol=1e-2))
        except Exception as e:
