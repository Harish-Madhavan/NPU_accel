...
class Attention(nn.Module):
    ...
    def forward(
        self, x, start_pos, freqs_cos, freqs_sin, mask, cache_k=None, cache_v=None
    ):
...
class Llama(nn.Module):
                ...
                nn.ModuleDict(
                    {
                        ...
                        "feed_forward": FeedForward(
                            params.dim, 4 * params.dim, params.multiple_of
                        ),
    ...
    def forward(self, tokens, start_pos, kv_cache=None):
        ...
        for i, layer in enumerate(self.layers):
            ...
            att_out, nk, nv = att(
                norm1(h), start_pos, freqs_cos, freqs_sin, mask, ck, cv
            )
            h = h + att_out
            h = h + ffn(norm2(h))
