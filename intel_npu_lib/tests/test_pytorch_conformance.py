...
class TestPyTorchConformance(unittest.TestCase):
    """
    ...
    def test_elementwise_autograd_conformance(self):
        ...
        for npu_op, torch_op, is_binary in ops_to_test:
            with self.subTest(op=torch_op.__name__):
    ...
    def test_activation_autograd_conformance(self):
        """Verify activations against PyTorch native F implementations."""
        x_cpu = torch.randn(3, 12, requires_grad=True)
        activations = [
        ...
        for npu_act, torch_act in activations:
            with self.subTest(act=torch_act.__name__):
