...
class TestAutogradStructural:
    def test_neg_autograd(self):
        ...
        assert torch.allclose(x2.grad, x1.grad, atol=1e-2, rtol=1e-2)
    ...
    def test_div_autograd(self):
        ...
        assert torch.allclose(a2.grad, a1.grad, atol=1e-2, rtol=1e-2)
        assert torch.allclose(b2.grad, b1.grad, atol=1e-2, rtol=1e-2)
    ...
    @pytest.mark.parametrize(
        "dim, keepdim",
        [(1, False), (1, True), ((1, 2), False), ((1, 2), True)],
    )
    def test_mean_autograd_dim_keepdim(self, dim, keepdim):
        x1 = torch.randn(2, 3, 4, requires_grad=True)
        x2 = x1.detach().clone().requires_grad_(True)
